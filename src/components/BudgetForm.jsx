...
const BudgetForm = ({ month, refresh }) => {
  ...
  const onSubmit = async (data) => {
    try {
      // isSubmitting from react-hook-form handles submit state; no need for separate loading state
      ...
      // Call refresh to reload budgets, assuming it only fetches data and does NOT submit form
      refresh();

      // Reset form AFTER successful submission and refresh
      reset();
    } catch (error) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
      ...
      <div className="flex items-center justify-between pt-2">
        <div className="text-sm text-gray-500">
          {month &&
            `Setting budget for ${new Date(month).toLocaleDateString("default", {
              month: "long",
              year: "numeric",
            })}`}
        </div>
